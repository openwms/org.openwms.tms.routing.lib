...
class RequestMessageConfiguration {
    ...
    @Bean("reqQueue")
    Queue queue(@Value("${owms.driver.req.queue-name}") String queueName) {
        return new Queue(queueName);
    }

    @Bean("reqBinding")
    Binding binding(
            @Value("${owms.driver.req.exchange-name}") String exchangeName,
            @Value("${owms.driver.req.queue-name}") String queueName,
            @Value("${owms.driver.req.routing-key}") String routingKey
    ) {
