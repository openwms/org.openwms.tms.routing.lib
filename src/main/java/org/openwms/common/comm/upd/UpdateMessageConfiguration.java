...
class UpdateMessageConfiguration {
    ...
    @Bean("updQueue")
    Queue queue(@Value("${owms.driver.upd.queue-name}") String queueName) {
        return new Queue(queueName);
    }

    @Bean("updBinding")
    Binding binding(
            @Value("${owms.driver.upd.exchange-name}") String exchangeName,
            @Value("${owms.driver.upd.queue-name}") String queueName,
            @Value("${owms.driver.upd.routing-key}") String routingKey
    ) {
